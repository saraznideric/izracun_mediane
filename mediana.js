...
document.getElementById("izracunaj-median").addEventListener("click", function () {
  ...
  axios.post("http://0.0.0.0:3000/api/mediana/calculate/", payload, options)
    .then(function (response) {
      return response
      throw new Error("Network response was not ok.");
    })
    .then(function (response) {
      prikaziTabeloMedianov(response.data.data);
    })
...
function naloziPodatke() {
  ...
  axios.get("http://0.0.0.0:3000/api/mediana/get/", options).
    then(function (response) {
      /*
      response je seznam
      */
      ...
      for (let i = 0; i < seznam.length; i++) {
        ...
        prikaziTabeloMedianov(seznam[i].MEDIANA);
      }
